...
router.get('/', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM scripts WHERE is_active = true ORDER BY sort_order, id');
...
router.get('/all', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM scripts ORDER BY sort_order, id');
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
